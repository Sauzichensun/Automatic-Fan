...
{  
	...
	for (j = 0; j < 8; j++)
	{
		...
		for(i = 0; i < 128; i++)
		{
			OLED_WriteData(0x00);//默认写入数据后，地址向右加1
		}
	}
}
...
/**
  * @brief  OLED显示中文，16*16
  * @param  Line 行位置，范围：1~4
  * @param  Column 列位置，范围：1~16
  * @param  Char 要显示的一个汉字，范围：自行定义字库
  * @retval 无
  */
void OLED_ShowCn(uint8_t Line, uint8_t Column, uint8_t num)
{
	uint8_t i;
	OLED_SetCursor((Line - 1) * 2, ((Column - 1)%16) * 8);		//设置光标位置在上半部分
	for (i = 0; i < 16; i++)
	{
		OLED_WriteData(OLED_FCN[num*2][i]);			//显示上半部分内容
	}
	OLED_SetCursor((Line - 1) * 2 + 1, ((Column - 1)%16) * 8);	//设置光标位置在下半部分
	for (i = 0; i < 16; i++)
	{
		OLED_WriteData(OLED_FCN[num*2][i + 16]);		//显示下半部分内容
	}
}

/**
  * @brief  OLED显示图像，16*16
  * @param  Line 行位置，范围：1~4
  * @param  Column 列位置，范围：1~16
  * @param  Char 要显示的一个汉字，范围：自行定义字库
	* @param  width 图像的宽度
...
void OLED_ShowMenuInfoFan(void)
{
	OLED_ShowString(1,1,"Grade:");
	if(fanstate)
	{
		OLED_ShowString(1,7,"On");
		OLED_ShowNum(1,9,fanstate,2);
	}
	else
	{
	...
	else OLED_ShowString(3,1,"Mode:Auto");
	OLED_ShowString(4, 12, "t:");
	OLED_ShowString(4, 4, "h:");
	OLED_ShowNum(4,6,temp,2);
	OLED_ShowChar(4,8,'.');
	OLED_ShowNum(4,9,tempfloat,2);
	OLED_ShowNum(4,14,humi,2);
